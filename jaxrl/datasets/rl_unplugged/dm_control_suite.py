...
from dm_control.composer.variation import colors, distributions
from dm_control.locomotion import arenas, props, tasks, walkers
...
def _build_rodent_escape_env():
    ...
    arena = arenas.bowl.Bowl(size=(20.0, 20.0), aesthetic="outdoor_natural")
    locomotion_task = tasks.escape.Escape(
        walker=walker, arena=arena, physics_timestep=0.001, control_timestep=0.02
    )
    raw_env = composer.Environment(
        time_limit=20, task=locomotion_task, strip_singleton_obs_buffer_dim=True
    )
...
def _build_rodent_maze_env():
    ...
    raw_env = composer.Environment(
        time_limit=30, task=rodent_task, strip_singleton_obs_buffer_dim=True
    )
...
def _build_rodent_two_touch_env():
    ...
    arena_floor = arenas.floors.Floor(size=(10.0, 10.0), aesthetic="outdoor_natural")
    task_reach = tasks.reach.TwoTouch(
        walker=walker,
        arena=arena_floor,
        target_builders=[
            functools.partial(props.target_sphere.TargetSphereTwoTouch, radius=0.025),
        ],
    ...
    raw_env = composer.Environment(
        time_limit=30, task=task_reach, strip_singleton_obs_buffer_dim=True
    )
...
def _build_humanoid_walls_env():
    ...
    raw_env = composer.Environment(
        time_limit=30, task=humanoid_task, strip_singleton_obs_buffer_dim=True
    )
...
class MujocoActionNormalizer(wrappers.EnvironmentWrapper):
    ...
    def step(self, action):
        ...
        if self._rescale == "tanh":
            ...
            scaled_actions = tree.map_structure(lambda a: np.clip(a, -1.0, 1.0), action)
        else:
...
class CmuThirdParty:
    ...
    def environment(self):
        ...
        self._environment = FilterObservationsWrapper(
            self._environment, all_observations
        )
...
def _padded_batch(example_ds, batch_size, shapes, drop_remainder=False):
    ...
    return example_ds.padded_batch(
...
def dataset(
    ...
    example_ds = example_ds.map(
        _build_sarsa_example, num_parallel_calls=tf.data.experimental.AUTOTUNE
    )
