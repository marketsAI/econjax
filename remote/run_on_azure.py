...
def run():
    ...
    local_mount = doodad.MountLocal(
        local_dir=TESTING_DIR, mount_point="/data", output=False
    )
    code_mount1 = doodad.MountLocal(
        local_dir="/home/kostrikov/GitHub/jax-rl/",
    ...
    doodad.run_command(
        command="python -u /code/jax-rl/train.py --env_name=HalfCheetah-v2 --max_steps=10000 --config=/code/jax-rl/configs/sac_default.py  --save_dir=/output/tmp/ > /output/output.out 2> /output/output.err",
        mode=launcher,
        mounts=mounts,
        verbose=True,
