...
flags.DEFINE_enum(
    "dataset_name", "d4rl", ["d4rl", "awac", "rl_unplugged"], "Dataset name."
)
...
flags.DEFINE_float(
    "percentile", 100.0, "Dataset percentile (see https://arxiv.org/abs/2106.01345)."
)
flags.DEFINE_float(
    "percentage", 100.0, "Pencentage of the dataset to use for training."
)
...
def main(_):
    ...
    video_save_folder = (
        None if not FLAGS.save_video else os.path.join(FLAGS.save_dir, "video", "eval")
    )

    env, dataset = make_env_and_dataset(
        FLAGS.env_name, FLAGS.seed, FLAGS.dataset_name, video_save_folder
    )
