...
def main(_):
    ...
    env_fns = [make_env(FLAGS.env_name, FLAGS.seed + i) for i in range(FLAGS.num_envs)]

    env = AsyncVectorEnv(env_fns=env_fns)
    eval_env = env_utils.make_env(FLAGS.env_name, FLAGS.seed + 42, video_eval_folder)
    ...
    agent = SACLearner(
        ...
        eval_env.action_space.sample()[np.newaxis],
        **kwargs,
    )
    ...
    for i in tqdm.tqdm(
                ...
                if done:
                    for k, v in info["episode"].items():
                        summary_writer.add_scalar(
                            f"training/{k}_{j}", v, total_timesteps
                        )
        ...
        if i % FLAGS.eval_interval == 0:
            ...
            for k, v in eval_stats.items():
                summary_writer.add_scalar(
                    f"evaluation/average_{k}s", v, total_timesteps
                )
            summary_writer.flush()
