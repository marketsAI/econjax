...
def get_T_autovec(model: dict):
    ...
    def T_autovec(v: jnp.ndarray):
        def action_v(a_ind: int, y_ind: int, ap_ind: int, v: jnp.array):
            c = (
                params["R"] * grids["a"][a_ind]
                + grids["y"][y_ind]
                - grids["ap"][ap_ind]
            )
            return jnp.where(
                c > 0,
                c ** (1 - params["gamma"]) / (1 - params["gamma"])
                + params["beta"] * jnp.dot(v[ap_ind, :], P[y_ind, :]),
                -jnp.inf,
            )
        ...
        all_state_v = jax.vmap(
            jax.vmap(one_state_v, in_axes=(None, 0, None, None)),
...
def main(
    output_file: str = os.getcwd() + "/benchmark_output.json", useTPU: bool = True
):
    import jax
    import jax.numpy as jnp
    ...
    print(output_file)
    # Global Parameteres
    params = {"R": 1.1, "beta": 0.99, "gamma": 2.5}
    a_min, a_max = 0.01, 2
    ρ = 0.9
    σ = 0.1
    ...
    for scale in [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024]:
        ...
        model = {
            ...
            "Trans_matrix": P,
            "indices": {
                "a": jnp.array(range(a_size)),
                "y": jnp.array(range(y_size)),
                "ap": jnp.array(range(ap_size)),
            },
        }
        ...
        global a_partitions
        a_partitions = jnp.reshape(
            model["indices"]["a"], (n_devices, a_size // n_devices)
        )
        T_tpu = get_T_tpu(model)
        global T_tpu_jit
        T_tpu_jit = (
            jax.pmap(T_tpu, in_axes=(0, None)).lower(a_partitions, v_init).compile()
        )
        results_dict["Manual Vectorization"].append(
            timeit.timeit(
                "T_manualvec_jit(v_init).block_until_ready()",
                globals=globals(),
                number=10,
            )
            / 10
        )
        results_dict["Automatic Vectorization"].append(
            timeit.timeit(
                "T_autovec_jit(v_init).block_until_ready()",
                globals=globals(),
                number=10,
            )
            / 10
        )
        results_dict["TPU Parallelization"].append(
            timeit.timeit(
